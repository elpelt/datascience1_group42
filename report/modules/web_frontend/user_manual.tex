...
A simple form is presented were dataset, distance measure and the k value can be chosen using drop-down menus or sliders. (Note: k in this case is not ne number of clusters. k stands for the k-nearest neighbour to any point)
Only when pressing the "Calculate kdist Graph" the sorted k-dist graph is calculated and plotted below.